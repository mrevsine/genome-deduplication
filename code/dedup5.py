...
def deduplicate_seq_retain_info(seq, seen_kmers, args):
    ...
    # Array and start pointer to retain info between samples
    sample_seq_info = np.array([0] * (sample_len * 2))
    seq_info_offset = 0
    ...
    for contig_start, contig_end, contig_N_regions in valid_contigs:
        ...
        while sample_start <= max_start_idx:
            ...
            if evaluation_method == "per_kmer":
                ...
                checked_sample_len, duplicate_start_idx, ignored_region, next_start_offset, sample_seen_kmers, sample_seq_info = \
                    check_sample_threshold_retain_info(seq[sample_start:sample_end], 
                                                       seq_info[seq_info_offset:(seq_info_offset+seqlen)], 
                                                       seq_info_offset, 
                                                       seen_kmers, 
                                                       k, 
                                                       min_sample_len, 
                ...
                # Must always be less than or equal to sample length, or else we will look at too much sequence and potentially run into issues with the end of the sequence    
