...
class SeqInfo:
	...
	def __init__(self, seqlen, k, encoding_dict=None):
		...
		self.n_repeat_kmers = 0
		# self.last_repeat_idx = None
	...
	def get_kmer_idx(self, kmer_num):
		...
		# print(f"Query returns {local_kmer_idx} on {decode_kmer(kmer_num)[:6]}... at global idx {self.current_global_idx} with seen kmers {[f"{decode_kmer(k)[:6]}..." for k in self.sample_seen_kmers.keys()]}")
		if local_kmer_idx == -1:
			return -1
		else:
		...
		if min_idx is None:
			...
			for kmer in kmers:
				...
				for i, pos in enumerate(kmer_positions):
					if pos >= self.current_global_idx + min_idx:
				...
				if first_safe_kmer_idx == len(kmer_positions):
					...
					newly_unique_idxs.append(kmer_positions[first_safe_kmer_idx])
	...
	# def get_next_repeat_idx(self, offset, seq):

	# 	# Get first index of a repeat
	# 	first_global_repeat_idx = None
	# 	first_local_repeat_idx = None
	# 	for i in range(offset, self.current_sample_length - self.k + 1):
	# 		if self[i] == self.encoding_dict["global repeat"]:
	# 			first_global_repeat_idx = i
	# 			break
	# 		elif self[i] == self.encoding_dict["internal repeat"]:
	# 			first_local_repeat_idx = i
	# 			break

	# 	# Global repeats are easy, return their index
	# 	if first_global_repeat_idx is not None:
	# 		return first_global_repeat_idx
		
	# 	# Local repeats are more complicated
	# 	# Find the position of the first instance of this local repeat
	# 	elif first_local_repeat_idx is not None:
	# 		repeat_kmer = seq[first_local_repeat_idx:first_local_repeat_idx+self.k]
	# 		repeat_kmer_num = encode_kmer(repeat_kmer)
	# 		first_local_repeat_kmer_idx = self.get_kmer_idx(repeat_kmer_num)
	# 		return first_local_repeat_kmer_idx
		
	# 	# If we don't find any repeats, return None
	# 	else:
	# 		return None


	# TODO: what order should these operations take place in?
	# TODO: need to account for the change in repeat bases when local repeat kmers become unique
	def move_internal_pointer(self, n_bases, seq, last_sample_accepted=False):

		if self.current_global_idx >= 13327000:
			print(f"Before move: first_repeat_idx = {self.first_repeat_idx}, annotation at first_repeat_idx = {self[self.first_repeat_idx] if self.first_repeat_idx is not None else 'NA'}")

		## Update the sample seen kmers
		## Drop any that are now passed by and update all indices relative to the new sample
		## Also, update the annotation of any kmers that are now unique 
		newly_unique_idxs = self.purge_kmers(None if last_sample_accepted else n_bases)
		for global_idx in newly_unique_idxs:
				local_idx = global_idx - self.current_global_idx
				self[local_idx] = self.encoding_dict["unique"]

		## At this point, all annotations from n_bases on are correct based on the new sample

		## Update repeat count data

		# If moving past the last possible annotation, simply reset all data
		if n_bases > self.n - self.k:
			# print(f"Moving {n_bases}; resetting n_repeat_bases = 0")
			self.n_repeat_bases = 0
			self.n_repeat_kmers = 0
			# self.last_repeat_idx = None
			self.last_repeat_end_idx = 0
		...
		# Otherwise, adjust all data based on the new position
		else:

			# Adjust needed relative indices (first and last repeat)
			if self.first_repeat_idx is not None:
				self.first_repeat_idx -= n_bases
				if self.first_repeat_idx < 0:
					# Find the new first repeat
					# Get new first repeat index with new coordinates
					new_first_repeat_idx = self.get_closest_repeat_idx(offset=n_bases) # The old coordinate system
					if self.current_global_idx >= 13_327_000:
						print(f"With adjustment of {n_bases}, old first_repeat_idx = {self.first_repeat_idx} (< 0), new first_repeat_idx is {new_first_repeat_idx}. seq_info[{new_first_repeat_idx}] = {self[new_first_repeat_idx] if new_first_repeat_idx >= 0 else 'NA'}, seq[new_first_repeat_idx:new_first_repeat_idx+self.k] = {seq[new_first_repeat_idx:new_first_repeat_idx+self.k] if new_first_repeat_idx >= 0 else 'NA'}, seen_kmers[kmer] = {self.sample_seen_kmers.get(encode_kmer(seq[new_first_repeat_idx:new_first_repeat_idx+self.k]), None) if new_first_repeat_idx >= 0 else 'NA'}")
					if new_first_repeat_idx is None:
						self.first_repeat_idx = None
					elif self[new_first_repeat_idx] == self.encoding_dict["global repeat"]:
						self.first_repeat_idx = new_first_repeat_idx - n_bases # Because it was the old coordinate system
					else: # Must be a local repeat, so need to find the position of its first match to get the correct first repeat index
						repeat_kmer = seq[new_first_repeat_idx:new_first_repeat_idx+self.k]
						repeat_kmer_num = encode_kmer(repeat_kmer)
						if self[new_first_repeat_idx] != self.encoding_dict["internal repeat"]:
							print(f"WARNING: new first repeat index is not a global or local repeat: index = {new_first_repeat_idx}, annotation = {self[new_first_repeat_idx]}, kmer = {repeat_kmer}, kmer indices = {[e - self.current_global_idx for e in self.sample_seen_kmers.get(repeat_kmer_num, [])]}")
						self.first_repeat_idx = self.get_kmer_idx(repeat_kmer_num) # The new adjusted index, since we already updated kmers
					
						if self.first_repeat_idx < 0:
							print(f"WARNING: global index = {self.current_global_idx}, first_repeat_idx = {self.first_repeat_idx}, kmer = {repeat_kmer}, kmer indices = {self.sample_seen_kmers.get(repeat_kmer_num, [])}.")

					# self.first_repeat_idx = self.get_next_repeat_idx(0, seq)
				
					if new_first_repeat_idx < 0:
						print(f"WARNING: global index = {self.current_global_idx}, new_first_repeat_idx = {new_first_repeat_idx}")


				# FOR DEBUGGING
				if self.current_global_idx >= 13_327_000:
					print(f"After adjusting for move of {n_bases}, new first_repeat_idx is {self.first_repeat_idx}. seq_info[{self.first_repeat_idx}] = {self[self.first_repeat_idx] if self.first_repeat_idx >= 0 else 'NA'}")
					print(f"Newly unique idxs (relative): {[idx - self.current_global_idx for idx in newly_unique_idxs]}")
					# print(seq[:100])

			self.last_repeat_end_idx = max(0, self.last_repeat_end_idx - n_bases) # Fine bc there will never be another repeat if this goes negative?
			# self.last_repeat_end_idx -= n_bases
			# if self.last_repeat_end_idx < 0:
			# 	self.last_repeat_end_idx = self.first_repeat_idx + self.k if self.first_repeat_idx is not None else 0

			# Get repeat kmers that we are moving past
			# Old coordinate system, but it doesn't matter, since we only care about the distance between repeats
			try:
				passed_kmers = np.where((self[:n_bases] == self.encoding_dict["internal repeat"]) | 
								(self[:n_bases] == self.encoding_dict["global repeat"]))[0]
			except IndexError:
				print(f"Current global index: {self.current_global_idx}")
				print(f"n_bases: {n_bases}, current_sample_length: {self.current_sample_length}, n: {self.n}")
				...
				for k,v in globals().items():
					print(f"{k}: {v}")
				exit()
			n_passed_kmers = len(passed_kmers)
			self.n_repeat_kmers -= n_passed_kmers

			# Based on the repeat kmers, compute the number of repeat bases that we are moving past
			n_passed_bases = 0
			if n_passed_kmers > 0:
				for i in range(n_passed_kmers - 1):
					curr_kmer_idx = passed_kmers[i]
					next_kmer_idx = passed_kmers[i+1]
					n_passed_bases += min(self.k, next_kmer_idx - curr_kmer_idx)

				# Find first repeat in the new range
				# These are relative to the adjusted range, so need to add n_bases to any index we find
				# next_kmer_idxs = np.where((self[n_bases:(min(self.n, n_bases+self.k))] == self.encoding_dict["internal repeat"]) |
				# 				(self[n_bases:(min(self.n, n_bases+self.k))] == self.encoding_dict["global repeat"]))[0]
				# next_kmer_idx = n_bases+self.k if len(next_kmer_idxs) == 0 else n_bases+next_kmer_idxs[0] # See above comment
				# Alternatively:
				# The old coordinate system, which is good bc it's consistent with the passed kmer indices
				next_kmer_idx = self.get_closest_repeat_idx(offset=n_bases, max_distance=self.k-1)
				if next_kmer_idx is None:
					next_kmer_idx = n_bases + self.k
				
				# Account for any bases of the last repeat in the old range that bleed into the new range,
				# up to where the first new repeat is
				distance_to_next_repeat = next_kmer_idx - passed_kmers[-1]
				n_passed_bases += min(self.k, distance_to_next_repeat)

			self.n_repeat_bases -= n_passed_bases
			# self.last_repeat_idx = max(-1, self.last_repeat_idx - n_bases)
			# self.last_repeat_idx = None if self.last_repeat_idx - n_bases < 0 else self.last_repeat_idx - n_bases
			
			# Update the repeat counts due to any kmers that used to be local repeats but are now unique 
			# due to the new sample not containing their original repeat matches
			# We previously accounted for all repeats passed by shifting forward n_bases,
			# now account for all repeats no longer present in the new sample past the shift
			newly_unique_idxs = [idx - self.current_global_idx for idx in newly_unique_idxs]
			further_repeat_kmer_count_decrement = len(newly_unique_idxs)
			self.n_repeat_kmers -= further_repeat_kmer_count_decrement
			further_repeat_base_count_decrement = 0
			for i in range(len(newly_unique_idxs)):
				global_idx = newly_unique_idxs[i]

				# Distance to next repeat is the maximum amount of bases defined as a repeat solely due to this kmer
				# This may be reduced if there is a previous repeat that also impacts some of these bases
				next_repeat = self.get_closest_repeat_idx(offset=global_idx, max_distance=self.k-1, forward=True, ignore_offset=True)
				next_newly_unique_idx = newly_unique_idxs[i+1] if i < len(newly_unique_idxs) - 1 else None
				if next_newly_unique_idx is not None and (next_repeat is None or next_newly_unique_idx < next_repeat):
					next_repeat = next_newly_unique_idx
				distance_to_next_repeat = self.k if next_repeat is None else min(self.k, next_repeat - global_idx)

				# Distance to the previous repeat lowers the amount of bases defined as repeats solely by this kmer
				previous_repeat = self.get_closest_repeat_idx(offset=global_idx, max_distance=self.k-1, forward=False, ignore_offset=True)
				distance_to_previous_repeat = self.k if previous_repeat is None else min(self.k, global_idx - previous_repeat)
				amount_covered_by_previous_repeat = self.k - distance_to_previous_repeat

				# Compute the number of repeat bases that are totally due to this kmer; these get subtracted
				n_repeat_bases_unique_to_this_kmer = max(0, distance_to_next_repeat - amount_covered_by_previous_repeat)
				further_repeat_base_count_decrement += n_repeat_bases_unique_to_this_kmer

			# Decrement all bases that are no longer repeats due to previous local repeat kmers now being unique
			# if further_repeat_base_count_decrement > 0:
			# 	print(f"Decrementing a further {further_repeat_base_count_decrement} repeat bases due to {further_repeat_kmer_count_decrement} local repeat kmers now being unique")
			self.n_repeat_bases -= further_repeat_base_count_decrement

		## Reset annotations for new kmers at the end of the new sample
		self.reset_annotations(n_bases)

		## Move forward the internal pointer for the start of the array
		self.array_start = self._resolve_index(n_bases)

		if self.current_global_idx >= 13327000:
			print(f"After move: first_repeat_idx = {self.first_repeat_idx}, annotation at first_repeat_idx = {self[self.first_repeat_idx] if self.first_repeat_idx is not None else 'NA'}")
			...
			# Unless we have already decided on the sample's fate in per-sample agnostic mode
				...
				# In per-sample agnostic mode, decide on the whole sample based on a coin flip when we reach the first offending kmer
					...
					if seq_info is None:
						...
						n_duplicate_kmers = seq_info.n_repeat_kmers
...
def deduplicate_seq(seq, seen_kmers, retain_info, args):
	...
	for contig_start, contig_end, contig_N_regions in valid_contigs:
		...
		while sample_start <= max_start_idx:
			...
			if seq_info is not None:
				seq_info.move_internal_pointer(next_start_offset, seq[sample_start:sample_end], last_sample_accepted=(checked_sample_len > -1))
