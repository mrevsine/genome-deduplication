...
class SeqInfo:
	...
	def __init__(self, seqlen, k, current_global_idx=0, encoding_dict=None):
		...
		self.kmer_encodings = np.zeros(seqlen, dtype=np.uint64) 
		self.kmer_presence = np.zeros(seqlen, dtype=bool)
		self.n = seqlen
	...
	def record_kmer_encoding(self, relative_idx, kmer_num):
		internal_idx = self._resolve_index(relative_idx)
		if self.kmer_presence[internal_idx]:
			raise ValueError(f"Error: trying to record kmer encoding at relative index {relative_idx} (internal index {internal_idx}) but there is already a kmer recorded there.")
		self.kmer_encodings[internal_idx] = kmer_num
		self.kmer_presence[internal_idx] = True
	...
	# 2. Record this kmer's encoding at this index
	# 3. Update repeat base count
	# 4. Possibly record this index as the first repeat for this sample
	def record_global_repeat_kmer(self, relative_idx, kmer_num):
		self[relative_idx] = self.encoding_dict["global repeat"]
		self.record_kmer_encoding(relative_idx, kmer_num)
	...
	# 3. Record this kmer's encoding at this index
	# 4. Update repeat base count
	# 5. Possibly record the index of the first instance of this local repeat as the first repeat for this sample
	...
	def update_seen_kmers(self, min_idx, passed_kmer_encodings):
		newly_unique_idxs = []
		if min_idx <= 0:
		...
		if min_idx > self.current_sample_length - self.k:
			...
			for kmer in passed_kmer_encodings:
				# These are all kmers, including global ones, so not all are guaranteed to be present
				# TODO
				if kmer not in self.sample_seen_kmers:
					continue
	...
	def update_coordinates(self, sample_start, sample_end, seq):
		...
		passed_kmer_encodings = set(self.kmer_encodings[self._resolve_slice(slice(0, n_advanced_bases))] if n_advanced_bases > 0 else np.array([]))
		...
		if n_advanced_bases > 0:
			...
			self.kmer_encodings[self._resolve_slice(slice(0, n_advanced_bases))] = 0
			self.kmer_presence[self._resolve_slice(slice(0, n_advanced_bases))] = False
		...
		newly_unique_idxs = self.update_seen_kmers(n_advanced_bases, passed_kmer_encodings)
			...
			## Retain info mode
				...
				# 4. If we don't already know what this kmer is, query it like normal
					...
					# 7. If not local repeat, check if global repeat
						...
						seq_info.record_global_repeat_kmer(kmer_start_idx, kmer_encoding)
						is_global_repeat = True
...
def deduplicate_genome(fasta, seen_kmers, args):
	...
	with open_maybe_gzip(fasta) as f:
		for record in SeqIO.parse(f, "fasta"):
			...
			print(f"	Deduplicating {seqname} with length {len(clean_sequence)}")
